...
3. Entraîner un Classificateur XGBoost pour prédire cette criticité.
4. Entraîner un Régresseur XGBoost pour prédire le score CVSS exact.
5. Évaluer et sauvegarder les modèles et les métriques de performance.
"""

# Désactivation des avertissements pour garder une sortie console propre
import warnings
warnings.filterwarnings('ignore')

# Librairies standards de manipulation de données
import numpy as np
import pandas as pd
import os

# Librairie pour la sauvegarde des modèles entraînés
import joblib

# Librairies de visualisation
import matplotlib.pyplot as plt

# Algorithme d'apprentissage automatique (Machine Learning) principal
import xgboost as xgb

# Utilitaires scikit-learn pour le prétraitement, l'évaluation et la calibration
...
# Librairie pour le sur-échantillonnage des classes minoritaires (gestion du déséquilibre)
...
# ÉTAPE 1 — Chargement
# ════════════════════════════════════════════════════════════════
DATASET_PATH = 'dataset_corrige.csv'
...
def assign_pentest_label(row):
    """
    Assigne un niveau de criticité (0=Low, 1=Medium, 2=High, 3=Critical) à une vulnérabilité.
    Cette fonction agit comme "l'expert métier" pour l'étiquetage des données.
    
    Règles basées sur la matrice de risque :
    - Le score CVSS brut.
    - La probabilité d'exploitation réelle (EPSS).
    - La disponibilité publique d'exploits (is_exploited, is_public).
    - L'exposition de l'infrastructure (serveur de base de données, sans authentification, etc.).
    """
    # Extraction des caractéristiques pour faciliter la lecture des règles
    ...
    av      = row['av_num'] # Vecteur d'accès (Network, Local, etc.)

    # --- NIVEAU 3 : CRITIQUE ---
    # Vulnérabilité exploitée, connue publiquement, avec un score CVSS > 7
    if exploit == 1 and public == 1 and score >= 7.0:   return 3
    # Exploit public existant et score CVSS très élevé
    if pexp == 1 and score >= 8.0:                      return 3
    # Probabilité d'exploitation (EPSS) forte (>15%), accessible via le réseau (av=4)
    if epss >= 0.15 and av == 4 and score >= 7.5:       return 3
    # Base de données exposée publiquement sans authentification requise
    if dbe == 1 and nav == 1 and score >= 8.0:          return 3
    # Machine extrêmement critique (crit=4) avec faille quasi-maximale
    ...
    # --- NIVEAU 0 : FAIBLE ---
    # Par défaut, si aucune des conditions aggravantes n'est remplie
    return 0
...
# Séparation initiale : 85% pour Train+Validation, 15% pour le Test final.
# L'argument `stratify=y_clf` garantit que la proportion des classes (Critique, Haute...) est maintenue.
X_tmp, X_test, yc_tmp, yc_test, yr_tmp, yr_test = train_test_split(
    X, y_clf, y_reg,
    test_size=0.15, random_state=RANDOM_STATE, stratify=y_clf
)

# Sous-séparation : Parmi les 85% restants, on réserve environ 15% du total global pour la Validation.
# (0.176 * 0.85 approx 0.15). Ainsi on obtient un split final : 70% Train, 15% Val, 15% Test.
...
# Objectif : éviter que le modèle ignore les classes minoritaires (souvent les failles "Critiques").
# On définit un seuil équivalent à 50% de la classe majoritaire.
minority_threshold = max(n_per_class.values()) * 0.5

# On définit la stratégie : si une classe a moins d'échantillons que le seuil, 
# SMOTE génèrera des données synthétiques jusqu'à atteindre ce seuil.
smote_strat = {
...
if smote_strat:
    # Création des échantillons synthétiques basés sur les 5 plus proches voisins
    ...
    # Si aucune classe n'est sous-représentée, on copie les données telles quelles
...
# Initialisation du modèle XGBoost pour la classification multiclasse.
clf_base = xgb.XGBClassifier(
    n_estimators=1000,       # Nombre d'arbres maximum
    max_depth=6,             # Profondeur maximale des arbres (limite le surapprentissage)
    learning_rate=0.02,      # Taux d'apprentissage (plus il est bas, plus le modèle est robuste mais lent)
    objective='multi:softprob', # Objectif : prédire des probabilités pour chaque classe
    num_class=n_classes,
    eval_metric='mlogloss',  # Métrique d'évaluation : log loss multiclasse
    ...
    min_child_weight=5,      # Poids minimum par feuille (régularisation)
    gamma=0.1,               # Réduction minimale de la perte requise pour faire une partition
    reg_alpha=0.1,           # Régularisation L1 (Lasso)
    reg_lambda=1.5,          # Régularisation L2 (Ridge)
    random_state=RANDOM_STATE, n_jobs=-1, verbosity=0,
    early_stopping_rounds=50 # Arrêt précoce : stoppe si aucune amélioration après 50 arbres
)

# Entraînement du modèle
clf_base.fit(
    X_sm, yc_sm,
    sample_weight=sw,                        # Pondération pour gérer le déséquilibre résiduel
    eval_set=[(X_sm, yc_sm), (X_val, yc_val)], # Suivi des performances en cours d'apprentissage
    verbose=False
)
...
# Les modèles à base d'arbres produisent souvent des probabilités mal calibrées (trop confiantes ou sous-évaluées).
# La calibration Isotonique ajuste les probabilités de sortie pour qu'elles reflètent la réalité statistique,
# en utilisant le jeu de validation (X_val).
clf_calibrated = CalibratedClassifierCV(
    xgb.XGBClassifier(
        n_estimators=best_iter, # On utilise le nombre optimal d'arbres trouvé à l'étape précédente
...
clf_calibrated.feature_names_in_ = np.array(FEATURES) # Enregistrement explicite du nom des colonnes
...
# Initialisation du Régresseur XGBoost (pour prédire le score CVSS brut).
# La configuration est similaire au classificateur, mais l'objectif est une erreur quadratique moyenne.
reg = xgb.XGBRegressor(
    n_estimators=1000, max_depth=5, learning_rate=0.02,
    objective='reg:squarederror', # Objectif : Minimiser l'erreur quadratique moyenne
    subsample=0.8, colsample_bytree=0.8,
    min_child_weight=5, gamma=0.1,
    reg_alpha=0.1, reg_lambda=1.5,
    random_state=RANDOM_STATE, n_jobs=-1, verbosity=0,
...
# Entraînement du régresseur sur les caractéristiques dédiées à la régression
reg.fit(
...
plt.tight_layout()
plt.savefig('graphiques/label_distribution.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.savefig('graphiques/confusion_matrix.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.savefig('graphiques/training_curves.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('graphiques/metrics_by_class.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('graphiques/roc_pr_curves.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('graphiques/feature_importances.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('graphiques/regressor_analysis.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('graphiques/ranking_metrics.png', bbox_inches='tight', dpi=300)
plt.show()
...
# --- Sauvegarde des Modèles (Exportation pour la production) ---
# Utilisation de joblib pour sérialiser (sauvegarder sur disque) les modèles entraînés.
# L'application (FastAPI/Backend) pourra ensuite recharger ces fichiers sans avoir besoin de ré-entraîner.
joblib.dump(clf_calibrated, 'models/classificateur_xgb.joblib')
joblib.dump(reg,            'models/regresseur_xgb.joblib')
joblib.dump(np.array(REG_FEATURES), 'models/reg_features.joblib') # Exporte aussi la liste des colonnes requises
